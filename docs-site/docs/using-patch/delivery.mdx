...
For storage backends, CDN options, and when to customize server adapters, see [Infrastructure adapters](/docs/setup/infrastructure). CDN setup: [Cloudflare](/docs/setup/cloudflare) or [CloudFront](/docs/setup/cloudfront). Most installs use the defaults from [Install](/docs/install).
