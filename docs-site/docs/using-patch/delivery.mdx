...
For storage backends, CDN options, and when to customize server adapters, see [Infrastructure adapters](/docs/setup/infrastructure). Cloudflare CDN setup: [Cloudflare setup](/docs/setup/cloudflare). Most installs use the defaults from [Install](/docs/install).
