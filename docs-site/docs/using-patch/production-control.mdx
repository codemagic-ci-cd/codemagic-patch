...
- Complete or disable the current rollout before publishing another update to the same deployment. `cmpatch release inspect --wait` only waits for the release worker; it does not finish a rollout
...
On the client, `mandatoryInstallMode` controls when that apply happens (default: `IMMEDIATE`). See [Applying updates](/docs/setup/applying-updates).
