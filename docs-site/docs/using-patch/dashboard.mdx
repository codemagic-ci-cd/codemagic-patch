...
After [Install](/docs/install), the dashboard is served from your API domain (e.g. `https://updates.example.com/`). Sign in with GitHub or Bitbucket OAuth, the same identity model as `cmpatch login`.
...
Open **Metrics** in the sidebar to browse rollups by app, then by deployment. The deployment detail and release pages also surface status (`uploaded`, `processing`, `published`, `failed`, `disabled`) and SDK-reported counters. For CLI equivalents, see [Analytics and metrics](/docs/using-patch/analytics).
