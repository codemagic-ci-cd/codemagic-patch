...
The install script can instead select external S3/GCS storage, and the guided
`cmpatch selfhost install` wizard offers Cloudflare or CloudFront delivery on a
first install. Splitting `MODE=api` / `MODE=worker` still requires a custom
...
External S3-compatible storage is selected at first install by running
`scripts/selfhost/install.sh` directly with its `--storage-mode s3` flags — the
guided `cmpatch selfhost install` wizard sets up bundled MinIO only. Changing
...
Select it at first install by running `scripts/selfhost/install.sh` directly
with `--storage-mode gcs` and separate public and internal buckets. The Compose
