...
With `cloudflare`, manifest URLs still use `PUBLIC_BASE_URL`, which should be the Cloudflare-fronted domain, and Patch purges cached objects so new releases propagate. Setup: [Cloudflare setup](/docs/setup/cloudflare).
