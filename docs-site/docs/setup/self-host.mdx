...
- writes `.env.selfhost` with strong random secrets for Postgres, MinIO, the worker, and the OAuth device-poll token (it refuses to overwrite an existing file),
...
Sign in to the dashboard with GitHub, then create apps and deployments (or use the CLI).
...
To front the storage domain with Cloudflare CDN and automatic cache purge after releases, complete [Cloudflare setup](/docs/setup/cloudflare) before install (DNS-only storage record during cert issuance, API token, install flags).
