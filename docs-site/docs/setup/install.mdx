...
For a quick try on your laptop without domains or TLS, start with the [Local quickstart](/docs/) (`./scripts/local-eval/up.sh`).
...
Dashboard and CLI sign-in use a GitHub OAuth App (`cmpatch login` signs in through the browser via the dashboard, so no extra app settings are needed for the CLI). [Register a new OAuth App](https://github.com/settings/applications/new) (or manage existing ones under [OAuth Apps](https://github.com/settings/developers)) and use:
...
:::info Bitbucket instead of GitHub
