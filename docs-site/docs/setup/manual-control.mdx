...
| `checkForUpdate()` | Returns `{ action: "up-to-date" \| "ota-update" \| "embedded-revert", remotePackage? }` plus `isStoreUpdateAvailable` and `latestBinaryVersion`. |
...
For when to check, install modes, and production UX patterns, see [Defining update checks](/docs/setup/defining-update-checks).
