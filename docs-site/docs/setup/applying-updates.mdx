...
For when the app checks and downloads updates, see [Checking for updates](/docs/setup/checking-for-updates). If you use `Patch.wrap(App, options)`, put the install options shown below in that wrapper call. The `sync()` examples are for direct integrations; adding a separate call does not configure the wrapper.
