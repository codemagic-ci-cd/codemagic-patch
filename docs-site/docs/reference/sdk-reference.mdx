...
For integration steps (native wiring, Expo plugin, first `sync()` call), see [Native setup](/docs/setup/native-setup) and [Update behavior](/docs/setup/update-behavior). For step-by-step control without `sync()`, see [Manual control](/docs/setup/manual-control).
...
Use during checkout, onboarding, or other flows where an unexpected reload would be disruptive. See [Update behavior](/docs/setup/update-behavior#blocking-restarts-during-critical-ux).
