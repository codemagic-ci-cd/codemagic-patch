...
For integration steps (native wiring, Expo plugin, first `sync()` call), see [Native setup](/docs/setup/native-setup) and [Defining update checks](/docs/setup/defining-update-checks). For step-by-step control without `sync()`, see [Manual control](/docs/setup/manual-control).
...
Calls `notifyAppReady()` first on every invocation. Uses `mandatoryInstallMode` when the remote release is mandatory; otherwise `installMode`. Defaults: optional → `ON_NEXT_RESTART`, mandatory → `IMMEDIATE`.
...
| --- | --- | --- | --- |
| `installMode` | `InstallMode` | `ON_NEXT_RESTART` | When optional updates apply |
| `mandatoryInstallMode` | `InstallMode` | `IMMEDIATE` | When mandatory updates apply |
...
Use during checkout, onboarding, or other flows where an unexpected reload would be disruptive. See [Defining update checks](/docs/setup/defining-update-checks#blocking-restarts-during-critical-ux).
