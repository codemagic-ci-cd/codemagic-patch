...
`/health` confirms the process is up. `/health/ready` also checks Postgres connectivity (returns `503` if the database is down). It does not probe object storage — check `https://<storage-domain>/minio/health/ready` separately.
...
- Release shows as active in the dashboard or `cmpatch release list`, but devices stay on the old bundle
...
2. After each **new native build**, run `release-react` with `--disabled --target-binary-version <version>` to record the fingerprint
...
1. **Cold-start twice** when testing: once to download, once to run the staged bundle
...
1. Reserve `--mandatory` for fixes that truly cannot wait
...
GitHub Actions and remote CI runners cannot call `http://127.0.0.1` or `localhost` on your laptop. Set `CODEMAGIC_PATCH_SERVER_URL` to a public Patch URL.
