...
2. For production, pick an install mode that matches your UX: `ON_NEXT_RESUME`, `ON_NEXT_SUSPEND`, or prompt the user then call `restartApp()`, see [Update behavior](/docs/setup/update-behavior#install-modes)
...
3. During critical flows (checkout, video call), use `disallowRestart()` / `allowRestart()` so an immediate mandatory update waits until the flow completes, see [Update behavior](/docs/setup/update-behavior#blocking-restarts-during-critical-ux)
