...
- At least one OAuth provider is configured: `GITHUB_OAUTH_CLIENT_ID` + `_SECRET` and/or `BITBUCKET_OAUTH_CLIENT_ID` + `_SECRET`
...
- `INITIAL_ADMIN_EMAILS` matches the GitHub (or Bitbucket) account's verified primary email
- OAuth callback URL is `https://<api-domain>/auth/callback` ([OAuth Apps](https://github.com/settings/developers))
