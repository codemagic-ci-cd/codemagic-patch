...
title: Core concepts
description: >-
  Apps, deployments, binary versions, and what can ship over the air versus in a
  store build.
llmsDescription: 'OTA concepts — apps, deployments, binary version, JS vs native.'
---
...
| Fingerprint | A hash of the native project recorded with releases. It actively widens delivery: the worker also publishes a release to other binary versions whose recorded fingerprint matches (fingerprint expansion), so store builds with identical native code receive the same update, while incompatible binaries stay excluded. |
