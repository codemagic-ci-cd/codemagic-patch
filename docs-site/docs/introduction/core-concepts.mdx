...
| Fingerprint | A hash of the native code in your project, recorded with releases. A later release whose fingerprint disagrees with the stored value is blocked until you confirm. Matching fingerprints can also widen delivery to other binary versions. See [Fingerprinting](/docs/using-patch/fingerprinting). |
