...
1. You publish a release with the CLI. It bundles your JS, computes a fingerprint, resolves a target binary version, and uploads to the API. The release worker then processes the upload and writes the bundle, patches, and manifest to object storage.
