 ...
 *
 * Vendored from Brad Conte's public-domain reference implementation:
 *   https://github.com/B-Con/crypto-algorithms (sha256.c / sha256.h)
 * Released into the public domain by the original author.
 *
 * Local changes: symbols renamed with a `codemagic_patch_` prefix, BYTE/WORD typedefs
 * replaced with <stdint.h> fixed-width types, and a streaming file-hash helper
 * (`codemagic_patch_sha256_file_hex`) added. The core compression function is
 * unmodified so it stays bit-identical to the standard reference.
 *
...
static void codemagic_patch_sha256_transform(codemagic_patch_sha256_ctx *ctx, const uint8_t data[]) {
...
void codemagic_patch_sha256_update(codemagic_patch_sha256_ctx *ctx, const uint8_t *data, size_t len) {
  for (size_t i = 0; i < len; ++i) {
    ...
    ctx->datalen++;
    if (ctx->datalen == 64) {
      codemagic_patch_sha256_transform(ctx, ctx->data);
      ctx->bitlen += 512;
      ctx->datalen = 0;
    }
  }
...
int codemagic_patch_sha256_file_hex(const char *path, char out_hex[65]) {
  ...
  while ((read_count = fread(buffer, 1, sizeof(buffer), file)) > 0) {
    codemagic_patch_sha256_update(&ctx, buffer, read_count);
  }
